    ...
    <div class="container py-5">
        ...
        <p class="h4 pb-4">
            @if ($user->servicio_id == null)
                Hola <b class="text-ennoia">{{$user->name}}</b>, ¿Quieres adquirir un servicio?
            @else
            Hola <b class="text-ennoia">{{$user->name}}</b>, ¿Quieres cambiar a un nuevo servicio?
            @endif
        </p>
        <div class="row gy-5">
            ...
            <div class="col-12 col-md-6 col-lg-4">
                <div class="card h-100 bg-ennoia-dark-light rounded-3 overflow-hidden text-white
                    ...
                    <div class="card-body text-center">
                        <h2 class="h4 py-2 card-title fw-bold text-ennoia text-capitalize">{{ $servicio->nombre }}</h2>
                        <p class="card-text">{{ $servicio->descripcion }}</p>
                    </div>
                    <div class="card-footer text-center">
                        <p class="py-2 card-title fw-bold text-ennoia h2">${{ $servicio->precio }}</p>
                    </div>
                    @auth
                    <div class="container my-2">
                        ...
                        <form action="{{ url('servicios') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="text-center">
                                ...
                                @else
                                    <a href="{{ url('/pagoServicio/'. $servicio->servicio_id ) }}" class="btn-ennoia d-inline-block mb-3">
                                        @if($user->servicio_id == $servicio->servicio_id )
                                        <i class="fa-solid fa-check"></i>
                                        @else
                                            Adquirir
